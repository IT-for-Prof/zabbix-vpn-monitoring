...
echo "=== TEST A2: zabbix user, ping -M do 1500B to 203.0.113.21 (PMTU 1492; expect: FAIL if DF honored) ==="
sudo -u zabbix ping -c2 -W2 -M do -s 1472 203.0.113.21 2>&1 | tail -3
echo "=== TEST A3: zabbix user, ping -M do 1492B to 203.0.113.21 (expect: OK) ==="
sudo -u zabbix ping -c2 -W2 -M do -s 1464 203.0.113.21 2>&1 | tail -2
echo
echo "=== TEST B: zabbix user wg show all dump (expect: needs root) ==="
sudo -u zabbix wg show all dump 2>&1 | head -2
echo "=== TEST B2: fping -M as zabbix to 203.0.113.21 @1500 vs @1492 ==="
echo -n "  @1500: "; sudo -u zabbix fping -M -b 1472 -c1 -t1500 203.0.113.21 2>&1 | tail -1
echo -n "  @1492: "; sudo -u zabbix fping -M -b 1464 -c1 -t1500 203.0.113.21 2>&1 | tail -1
echo
...
sudo -u zabbix cat /sys/class/net/wg0/mtu 2>&1
echo "=== END ==="
